...
{
    //
    // MicroNT: csrss-free.  Original kernel32 LPC'd basesrv via
    // BasepDefineDosDevice, which then manipulated the \?? object
    // directory under SYSTEM context.  Without csrss we'd need a
    // privileged daemon (or kernel-side ACL) to mutate \??\<name>;
    // none exists yet.  Fail loud so callers don't think a drive
    // letter mapping silently succeeded.  Toolchain does not call
    // this API.  Re-implement against direct NtCreateSymbolicLink
    // when a real consumer arrives.
    //
    UNREFERENCED_PARAMETER( dwFlags );
    UNREFERENCED_PARAMETER( lpDeviceName );
    UNREFERENCED_PARAMETER( lpTargetPath );
