...

{
    ...
    try {
        ...
        // debugger.  Use DbgPrint to send it to the kernel
        // debugger.  DbgPrint can only handle 511 characters
        // at a time, so force-feed it.
        //
        ...
        try {
            ...
            while (cchRemaining > 0) {
                if (cchRemaining >= sizeof(szBuf)) {

                    RtlCopyMemory(szBuf, pszRemainingOutput, sizeof(szBuf)-1);
                    szBuf[sizeof(szBuf)-1] = 0;

                    DbgPrint("%s", szBuf);

                    pszRemainingOutput += sizeof(szBuf)-1;
                    cchRemaining       -= sizeof(szBuf)-1;

                    }
                else {
                    DbgPrint("%s", pszRemainingOutput);
                    cchRemaining = 0;
                    }
                }
            }
...

{
    ...
    m = RtlAllocateHeap(RtlProcessHeap(), 0,sizeof(*m));
    if ( !m ) {
