...
Routine Description:
    ...
    An application thread may delete an un-owned critical section using
    Win32DeleteCriticalSection.

    Deleting a critical section deallocates all system resources stored
    in the critical section.  Once deleted, the critical section may not
    be used in Win32EnterCriticalSection or Win32LeaveCriticalSection.

    In order to delete a critical section, the critical section must not
    be owned.
...
--*/
{
    ...
    if (nCount > 8) {
        HandleArray = (LPHANDLE) RtlAllocateHeap(RtlProcessHeap(), 0, nCount*sizeof(HANDLE));
        if (HandleArray == NULL) {
