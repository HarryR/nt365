...

{
    ...
    if ( !RtlTimeFieldsToTime(&TimeFields,&SystemTime) ) {
        ...
        Status = BasepAcquirePrivilege( SE_SYSTEMTIME_PRIVILEGE, &State );
...

{
    ...
    Result.QuadPart = FileTime1.QuadPart - FileTime2.QuadPart;
    if (Result.HighPart < 0) {
        return( -1 );
        }
    else
    if (Result.LowPart == 0 && Result.HighPart == 0) {
        return( 0 );
        }
    else {
        return( 1 );
...

{
    ...
    TimeAdjust.Enable = bTimeAdjustmentDisabled;
