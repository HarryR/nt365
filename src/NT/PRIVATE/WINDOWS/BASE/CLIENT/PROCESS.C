...
BasepIsImageVersionOk(
    IN ULONG ImageVersion
    )
{
    ULONG ImageMinorVersion;
    ULONG ImageMajorVersion;

    ImageMajorVersion = ImageVersion & 0x000000ff;
    ImageMinorVersion = (ImageVersion & 0x00ff0000) >> 16;
    ...
    if ( ( ImageMajorVersion > BaseWindowsMajorVersion ) ||
         ( ImageMajorVersion == BaseWindowsMajorVersion && ImageMinorVersion > BaseWindowsMinorVersion ) ) {

        //
        // even though 4.0 is greater than our format, allow 4.0 since this is what
        // the chicago linker does
        //

        if ( ImageMajorVersion == 4 && ImageMinorVersion == 0 ) {
            return TRUE;
            }
        else {
            return FALSE;
...
VdmRetry:
        ...
        if (!ARGUMENT_PRESENT( lpApplicationName )) {
            ...
            NameBuffer = RtlAllocateHeap(RtlProcessHeap(), 0,(MAX_PATH<<1));

            lpApplicationName = lpCommandLine;
            TempNull = (PWCH)lpApplicationName;
            while(*lpApplicationName) {
                if ( *lpApplicationName == (WCHAR)' ' ||
                     *lpApplicationName == (WCHAR)'\t' ) {
                    TempNull = (PWCH)lpApplicationName;
                    break;
                    }
                lpApplicationName++;
...
itsok:
                        ...
                        AnsiCmdCommand.Buffer = RtlAllocateHeap(RtlProcessHeap(), 0,
        ...
        if ( ImageInformation.SubSystemType != IMAGE_SUBSYSTEM_WINDOWS_GUI &&
            ...
            if (!BasepIsImageVersionOk(ImageInformation.SubSystemVersion) ) {
                SetLastError(ERROR_BAD_EXE_FORMAT);
                return FALSE;
                }
            }
        ...
        if (ImageFileDebuggerCommand[ 0 ] != UNICODE_NULL) {
            ...
            if (n == 0) {
                DbgPrint( "BASE: CreateProcess called with zero length command line.  lpCommandLine == '%ws' (%u)\n", lpCommandLine, n );
                lpCommandLine = lpApplicationName;
                n = wcslen( lpCommandLine );
                DbgPrint( "BASE: Using ApplicationName instead.  lpCommandLine == '%ws' (%u)\n", lpCommandLine, n );
                DbgBreakPoint();
                }
            ...
            SubSysCommandLine.Buffer = RtlAllocateHeap( RtlProcessHeap(), 0, n );
        ...
        if ( ARGUMENT_PRESENT(lpCurrentDirectory) ) {
            ...
            if ( CurdirLength ) {
                CurdirBuffer = RtlAllocateHeap(RtlProcessHeap(), 0,CurdirLength+sizeof(UNICODE_NULL));
                if ( !CurdirBuffer ) {
                    BaseSetLastNTError(STATUS_NO_MEMORY);
                    return FALSE;
                    }
        ...
        if ( ImageInformation.SubSystemType == IMAGE_SUBSYSTEM_WINDOWS_GUI ) {
            a->ProcessHandle = (HANDLE)((DWORD)ProcessHandle | 2);
            ...
            if ( NtHeaders->OptionalHeader.Subsystem == IMAGE_SUBSYSTEM_WINDOWS_GUI ) {
                ...
                // window manager to provide feedback through the
                // cursor for gui apps. Use the 1 bit for this.
                //
                a->ProcessHandle = (HANDLE)((DWORD)ProcessHandle | 3);
                }
            }
...

{
    ...
    try {
        if ( !BaseAnsiStartupInfo ) {
            BaseAnsiStartupInfo = RtlAllocateHeap(RtlProcessHeap(), 0,sizeof(*BaseAnsiStartupInfo));
            if (!BaseAnsiStartupInfo) {
...

{
    ...
    // Go for worst case
    Buffer.Buffer = RtlAllocateHeap(RtlProcessHeap(), 0, (cch+2)*sizeof(WCHAR));
...

{
    ...
    UnicodeValue.MaximumLength = (USHORT)(nSize ? nSize - 1 : nSize)*sizeof(WCHAR);
    UnicodeValue.Buffer = (PWCHAR)
	RtlAllocateHeap( RtlProcessHeap(), 0, UnicodeValue.MaximumLength );
	...
	Value.MaximumLength = (USHORT)nSize;
    ...
    )
{
    ...
    Value.Buffer = lpBuffer;
    Value.Length = 0;
    Value.MaximumLength = (USHORT)(nSize ? nSize - 1 : nSize)*sizeof(WCHAR);
    ...
    )
{
	...
	*lpDst = '\0';
    ...
    UnicodeDest.MaximumLength = (USHORT)(nSize ? nSize - 1 : nSize)*sizeof(WCHAR);
    UnicodeDest.Buffer = (PWCHAR)
	RtlAllocateHeap( RtlProcessHeap(), 0, UnicodeDest.MaximumLength );
...

{
    ...
    try {
        ...
        NameBuffer = RtlAllocateHeap(RtlProcessHeap(), 0,MAX_PATH);
        ...
        CommandLineBuffer = RtlAllocateHeap(RtlProcessHeap(), 0,(ULONG)LoadModuleParams->lpCmdLine[0]+1+Length+1);
...
Arguments:
    ...
    lpSystemInfo - Returns information about the current system.
        ...
        DWORD dwOemId - Returns an oem specific machine id.
        ...
        DWORD dwProcessorType - Returns the type of the current
            processors in the system.  All processors are assumed to be
            of the same type, stepping, and are configured with the same
            options.
...
--*/
{
    ...
    lpSystemInfo->dwOemId = BasicInfo.OemMachineId;
    ...
    lpSystemInfo->dwProcessorType = ProcessorInfo.ProcessorType;
    lpSystemInfo->dwAllocationGranularity = BasicInfo.AllocationGranularity;
    lpSystemInfo->dwReserved = 0;
    ...
    )
{
    ...
    SubSysCommandLine->Buffer = RtlAllocateHeap( RtlProcessHeap(), 0,
