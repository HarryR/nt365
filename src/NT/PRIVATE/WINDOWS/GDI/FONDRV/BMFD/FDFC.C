...
#ifdef DBCS_VERT // ROTATION: ulGetRotate() function body

/******************************Private*Routine*****************************\
*
* STATIC VOID vComputeRotatedXform()
*
* History :
*
...
#ifndef DBCS_VERT // We use vComputeRotatedXform() instead of vInitXform()
...
#ifdef DBCS_VERT // BmfdOpenFontConText(): Compute cjGlyphMax
...
// init stretch flags
    ...
    {
#ifdef DBCS_VERT // BmfdOpenFontContext() Adjust stretch buffer
...
#ifdef DBCS_VERT // BmfdOpenFontContext() keep rotation degree in FONTCONTEXT
...
// opening another context off of the same fontfile pff
    ...
    // and if this is a 16BITDLL or an FNT file, than we have to remap
        ...
        if (pff->cRef == 0)
        {
            ...
            for (i = 0; i < (INT)pff->cFntRes; i++)
            {
                pff->afai[i].re.pvResData = (PVOID) (
                    (BYTE*)pff->u.fvw.pvView + pff->afai[i].re.dpResData
                    );
            }
        }
    }
    ...
    )
{
    ...
    ASSERTGDI(hfc, "BmfdCloseFontContext, hfc\n");
...
// closing  another context off of the same fontfile pff
    ...
    {
        RET_FALSE("BmfdCloseFontContext: cRef <= 0\n");
    }
