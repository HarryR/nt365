...
*
* History:
*  25-Oct-1990 -by- Bodin Dresevic [BodinD]
* Wrote it.
\**************************************************************************/

BOOL
bBmfdLoadFontFile(
    PWSZ pwszFileName, HFF *phff
    )
{
    FILEVIEW fvw;
    BOOL     bRet;
...
// Map file to memory.
    ...
    {
        WARNING("BMFD!BmfdLoadFontFile(): error memory mapping file\n");
        return(HFF_INVALID);
    }

// try loading it as an fon file, if it does not work, try as fnt file

    if (!(bRet = bBmfdLoadFont(pwszFileName, &fvw,TYPE_DLL16, phff)))
    {
    // try as an *.fnt file

        bRet = bBmfdLoadFont(pwszFileName, &fvw,TYPE_FNT,phff);
    }

// unmap the file, we will not need untill it is first used
...
// if this did not work try to load it as a 32 bit dll
    ...
    if (!bRet)
    {
        bRet = bLoadNtFon(pwszFileName,phff);
    }
...
// separate loops for faster processing:
    ...
    if (PFF(hff)->iType == TYPE_DLL32)
    {
        while (pfai < pfaiTooFar)
        {
            vUnloadGlyphset(ghsemBMFD, &gpcpGlyphsets, pfai->pcp);
            vUnlockAndFreeResource(pfai->hResData);
            pfai += 1;
        }
    }
    else // memory mapped files, no need to unlock resources
    {
        while (pfai < pfaiTooFar)
        {
            vUnloadGlyphset(ghsemBMFD, &gpcpGlyphsets, pfai->pcp);
...
// unmap the file
    ...
    switch(PFF(hff)->iType)
    {
    case TYPE_DLL16: // no need to unmap, that happened when cRef went to zero
    case TYPE_FNT:   // no need to unmap, that happened when cRef went to zero
    case TYPE_EXE:   // LoadFontResData loaded this one
        break;

    case TYPE_DLL32:
        vFreeLibrary(PFF(hff)->u.hMod);
        break;
    default:
        RIP("bmfd!_ incorrect type \n");
        break;
    }
...
// The iVersion only had length of 2 bytes in the original struct
    ...
    pcvtfh->iVersion = (ULONG)usMakeUSHORT((PBYTE)&ajHdr[OFF_Version      ]);
...
// chFirst + chDefault(Break)
    ...
    // chars screwed up, but everything else is ok. Example of such shipped
...
{
    PFONTFILE   pff;
    ULONG       cjff, dpwszFileName;
    WINRESDATA  wrd;
    RES_ELEM    re;
    ULONG       ifnt;
    SIZE_T      cjDescription;      // size of the desctiption string (in bytes)
...
// Otherwise, if .FNT format, the current file view may be used.
    ...
    {
        re.pvResData = pfvw->pvView;
        re.dpResData = 0;
        re.cjResData = pfvw->cjView;
...
// We assume font all font resource is SHIFT_JIS font. We prepare room for Vertical font
    ...
    if ((iType == TYPE_DLL16) && bDescStr(pfvw, achDescription))
    {
...
// allocate mem at the bottom for the file name:

    dpwszFileName = cjff;
    cjff += (sizeof(WCHAR) * (wcslen(pwszFileName) + 1));
...
// Allocate a FONTFILE of the appropriate size from the handle manager.
    ...
    if ((*phff = hffAlloc(cjff)) == HFF_INVALID)
    {
        ...
        RETURN("bmfd!hffLoadFont(): memory allocation error\n", FALSE);
    }
...
// Init fields of pff structure
    ...
    pff->fl         = 0;
    pff->iType      = iType;
    pff->u.fvw      = *pfvw;        // will be needed at unload time
...
//        pff->afai[cFontResIncludeVert].bVertical = NON_VERTICAL;
//   !!! fix this gerritv
        ...
        //
        // Check this font resource's Charset is SHIFTJIS_CHARSET. If it is so , Set up
        // Vertical font stuff. Or not so. Increment counter.
        //

//        if( (pff->afai[cFontResIncludeVert - 1].cvtfh.usCharSet) == SHIFTJIS_CHARSET )
//      !!! fix this [gerritv]
        if( FALSE )
        {
...
//            pff->afai[cFontResIncludeVert].bVertical = VERTICAL;
// !!!  fix this gerritv
//
...
// compute glyph set that corresponds to this resource:
    ...
    if ((pfai->pcp = pcpComputeGlyphset(
                          ghsemBMFD,
                          &gpcpGlyphsets,
    ...
    // If we fail it should be because of we are out of memory.
       ...
       SAVE_ERROR_CODE(ERROR_NOT_ENOUGH_MEMORY);
       RET_FALSE("BMFD!bConvertFontRes(): memory allocation error\n");
    }
...
//Only one thread can be working on the list at a time.
    ...
    //
        ...
        if (( pcpTmp =  (CP_GLYPHSET*)
              (PALLOCNOZ((SZ_GLYPHSET(cNumRuns,cGlyphs) +
                         offsetof(CP_GLYPHSET,gset)),BMFD_TYPE))
            ) == (CP_GLYPHSET*) NULL)
        {
...
#ifdef DBCS_VERT // VERTICAL:cjIFIMETRICS(): make room for '@'
...
// start searhing backwards for the bottom
    ...
    )
    {
        if (bNonZeroRow(pjRow, cy, cjWidth))
        {
            *pyBottomExcMax = ((pjRow - pjBitmap) + 1); // + 1 for exclusiveness
            break;
        }
    }
...
/******************************Public*Routine******************************\
*
* VOID vComputeGLYPHDATA
*
* computes glyph metrics of the glyph by looking at the bitmap in the
* native column format.
*
* History:
*  05-Oct-1992 -by- Bodin Dresevic [BodinD]
* Wrote it.
\**************************************************************************/
...
#ifdef DUMPCALL
    DbgPrint("\nvComputeGLYPHDATA(\n");
    DbgPrint("    GLYPHDATA  *pgldt    = %-#8lx\n",pgldt);
    DbgPrint("    PBYTE       pjBitmap = %-#8lx\n",pjBitmap );
    DbgPrint("    ULONG       cx       = %d\n", cx          );
    DbgPrint("    ULONG       cy       = %d\n", cy          );
    DbgPrint("    ULONG       yBaseLine= %d\n", yBaseLine   );
    DbgPrint("    );\n\n"                                   );
#endif
...
// character increment vector is always equal to (cx,0) since character origin
// for every character is defined as the leftmost pixel on the baseline.
...
// inked rectangle has to be expressed in notional coords
    ...
    // and last C are taken into account
        ...
        yBottomExcMax = cy; // coincides with the bottom
    }

// cheat on left and right edge, not important for positioning

    pgldt->rclInk.left = (LONG)0;
    pgldt->rclInk.right = (LONG)cx;
...
// compute bearings, remember the rule A + B + C == char inc == cx,
// for horizontal, left to right writing:
//             A == ePreBearing,  C = ePostBearing,

// cheating about ABC spaces when no simulations, win31 compatibility

    pgldt->fxA = 0;             // fxA
    pgldt->fxAB = pgldt->fxD;   // right edge of the black box
...
//
    cchFaceName = lstrlen(pszFaceName);
    pifi->dpwszStyleName =
...
#ifdef DBCS_VERT // vBmfdFill_IFIMETRICS():Add @ to face name

//    if( pfai->bVertical == VERTICAL )
// !! fix this gerritv
    if( pfai->bVertical == TRUE )
    {
...
#ifdef DBCS_VERT // vBmfdFill_IFIMETRICS():set FM_INFO_90DEGREE_ROTATIONS flag

// we can rotate bitmap font to 90 , 180 , 270 degree. font driver inform this function
// to graphic engine via this flag
//                                        [hideyukn]
    ...
    )
{
    ...
    COUNT cch;              // count of characters in string resource
    PSZ psz;                // pointer to characters in string resource
    PSZ pszTmp;
    PBYTE pj = (PBYTE) pfvw->pvView;    // PBYTE pointer into file view

#ifdef DUMPCALL
    DbgPrint("\nbDescStr(\n"                            );
    DbgPrint("    PFILEVIEW pfvw = %-#8lx\n", pfvw      );
...
// Validation.  Check EXE_HDR magic number.
    ...
    if (READ_WORD(pj + OFF_e_magic) != EMAGIC)
        RET_FALSE("bmfd!bDescStr(): not a 16-bit .FON file (bad EMAGIC number)!\n");
...
// More validation.  Check NEWEXE magic number.
    ...
    if (READ_WORD(pj + dpNewExe) != NEMAGIC )
        RET_FALSE("bmfd!bDescStr(): not a 16-bit .FON file (bad NEMAGIC number)!\n");
...
// guaranteed.  However, we know the string is < 256 characters.
    ...
    if ( (cch = (COUNT)(*(pj + dpNRSTOffset))) == 0 )
    {
...
// If cx is non-zero, then the character exists, else use default char
    ...
    if (*pus_cx == 0)
    {
        pus_cx = pusOffTable + pcvtfh->chDefaultChar * cusTableEntry;
        ASSERTGDI(*pus_cx, "bmfd!_ default char does not exhist\n");
    }
    ...
    )
{
    ...
    PUSHORT  pus_cx, pus_cxTmp;  // pointer to the beginning of the offset table
...
// make sure that there is no bug in the font file header such that
// pcvtfh->chDefaultChar points to a char that is not supported in
// this font file. If there happens to be such a bug, find the first
// supported char and promote it to the default char. That way the system
// will not crash, it may just display a bogus char instead of intended
// (by font designer) default char. This is
// extremely unlikely and it may only happen in some special purpose fonts.

    if ((*(pus_cx + (pcvtfh->chDefaultChar * cusTableEntry))) == 0)
    {
    // this is a bug in the font file, we shall fix it now:

        pus_cxTmp = pus_cx;

        for (i = 0; i < cCodePoints; i++, pus_cxTmp += cusTableEntry)
        {
            if (*pus_cxTmp)
            {
                pcvtfh->chDefaultChar = (UCHAR)i;
                break; // the problem is fixed now
            }
        }
    }
