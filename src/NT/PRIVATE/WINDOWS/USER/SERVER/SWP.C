...
* 01-25-91 IanJa      added window revalidation
* 07-11-91 darrinm    Replaced everything with re-ported Win 3.1 code.
\***************************************************************************/
...
BOOL ValidateWindowPos(PWINDOWPOS ppos);
BOOL ValidateZorder(PCVR pcvr);
int BltValidInit(PSMWP psmwp);
BOOL SwpCalcVisRgn(PWND pwnd, HRGN hrgn);
BOOL CombineOldNewVis(HRGN hrgn, HRGN hrgnVisOld, HRGN hrgnVisNew,
        UINT crgn, UINT fsRgnEmpty);
PWND PWInsertAfter(HWND hwnd);
HWND HWInsertAfter(PWND pwnd);
BOOL xxxEndDeferWindowPosEx(PSMWP psmwp, BOOL fEvent);
...
BOOL xxxBringWindowToTop(
    PWND pwnd)
{
    CheckLock(pwnd);

    return xxxSetWindowPos(pwnd, (PWND)HWND_TOP, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
}
...
*
* History:
* 07-25-91 darrinm      Ported from Win 3.1 sources.
\***************************************************************************/
    ...
    BOOL fRedraw)
{
    ...
    if (pwnd == PWNDDESKTOP(pwnd) || TestWF(pwnd, WFWIN31COMPAT) ||
        ...
        return xxxSetWindowPos(pwnd, NULL, x, y, cx, cy, (fRedraw ?
         ...
         */
        BOOL fResult = xxxSetWindowPos(pwnd, NULL, x, y, cx, cy,
                SWP_NOZORDER | SWP_NOACTIVATE);
    ...
    int cwndHint)
{
    ...
    acvr = (PCVR)LocalAlloc(LPTR, sizeof(CVR) * cwndHint);

    if (acvr == NULL)
        return NULL;

    psmwp = (PSMWP)CreateObject(PtiCurrent(), TYPE_SETWINDOWPOS,
            sizeof(SMWP), NULL, NULL);
    ...
    UINT flags)
{
    ...
    if (psmwp->ccvr + 1 > psmwp->ccvrAlloc) {
         ...
         */
        psmwp->ccvrAlloc += 4;
        pcvr = (PCVR)LocalReAlloc(psmwp->acvr,
                sizeof(CVR) * psmwp->ccvrAlloc, LPTR | LMEM_MOVEABLE);
        if (pcvr == NULL) {
            CloseObject(psmwp);
            return NULL;
        }
    ...
    ppos->hwnd = HW(pwnd);
    ppos->hwndInsertAfter = HWInsertAfter(pwndInsertAfter);
...
*
* History:
* 07-10-91 darrinm      Ported from Win 3.1 sources.
\***************************************************************************/
...
{
    PCVR pcvr;
    PWND pwndParent, pwndT;
    int ccvr;

    *pfSyncPaint = TRUE;

    pwndT = ValidateHwnd(psmwp->acvr[0].pos.hwnd);
    if (pwndT == NULL)
        return FALSE;
    ...
    for (pcvr = psmwp->acvr, ccvr = psmwp->ccvr; --ccvr >= 0; pcvr++) {
        ...
        if (PW(pcvr->pos.hwnd)->spwndParent != pwndParent) {
            SetLastErrorEx(ERROR_HWNDS_HAVE_DIFF_PARENT, SLE_ERROR);
            return FALSE;
        }
    ...
    HWND *phwndNewActive)
{
    ...
    int cxSrc, cySrc, cxDst, cyDst, cmd, ccvr;
    int xClientOld, yClientOld, cxClientOld, cyClientOld;
    int xWindowOld, yWindowOld, cxWindowOld, cyWindowOld;
    TL tlpwndParent;
    TL tlpwnd;
    ...
    for (pcvr = psmwp->acvr, ccvr = psmwp->ccvr; --ccvr >= 0; pcvr++) {
         ...
         */
        if (!(pcvr->pos.flags & SWP_NOMOVE) && !TestWF(pwnd, WFWIN31COMPAT) &&
         ...
         */
        if (((pcvr->pos.flags & (SWP_NOSIZE | SWP_FRAMECHANGED))
                != SWP_NOSIZE) || fForceNCCalcSize) {
            WINDOWPOS pos;
...
AllInvalid:
                 ...
                 */
                pcvr->rcBlt.left = pcvr->rcBlt.top =
                pcvr->rcBlt.right = pcvr->rcBlt.bottom = 0;
    ...
    UINT flags)
{
    ...
    if ((psmwp = _DeferWindowPos(psmwp, pwnd, pwndInsertAfter,
            x, y, cx, cy, flags)) == NULL) {
        return FALSE;
    }
    ...
    PWND pwndNewActive)
{
    ...
    if (TestwndChild(pwndNewActive)) {
        ...
        if (pti->pq == gpqForeground && pti != GETPTI(pwndNewActive)) {
            ...
            GETPTI(pwndNewActive)->flags |= TIF_ALLOWFOREGROUNDACTIVATE;
        }
...
{
    BOOL fFinished;
    PCVR pcvrFirst, pcvr, pcvrT;
    int ccvrRemaining, ccvr, chwnd;
    PTHREADINFO ptiT, ptiCurrent;
    PSMWP psmwpNew;
    ...
    while (TRUE) {
         ...
         */
        psmwpNew = (PSMWP)LocalAlloc(LPTR, sizeof(SMWP) + (sizeof(CVR) * chwnd));
         ...
         */
        PostEventMessage(ptiT, ptiT->pq, (DWORD)psmwpNew, (LONG)ptiT,
                QEVENT_SETWINDOWPOS);
    }
    ...
    PSMWP psmwpT)
{
     ...
     * and later free.
     */
    psmwp = _BeginDeferWindowPos(psmwpT->ccvr);
    if (psmwp == NULL) {
        LocalFree(psmwpT);
        return;
    }
...
BOOL ValidateWindowPos(
    PWINDOWPOS ppos)
{
    PWND pwnd, pwndInsertAfter;
    HWND hwndInsertAfter;

    pwnd = RevalidateHwnd(ppos->hwnd);
    if (pwnd == NULL)
        return FALSE;
    ...
    if (!(ppos->flags & SWP_NOZORDER)) {
        ...
        hwndInsertAfter = ppos->hwndInsertAfter;
        ...
        if (hwndInsertAfter != HWND_TOP && hwndInsertAfter != HWND_BOTTOM) {
            ...
            if (((pwndInsertAfter = RevalidateHwnd(hwndInsertAfter)) == NULL) ||
                ...
                SRIP1(ERROR_INVALID_HANDLE, "Invalid hwndInsertAfter (0x%lx)",
                        hwndInsertAfter);
                return FALSE;
            }
            ...
            if (pwnd == pwndInsertAfter ||
                ...
                SRIP2(RIP_WARNING, "hwndInsertAfter (0x%lx) is not a sibling "
                        "of hwnd (0x%lx)", hwndInsertAfter, ppos->hwnd);
                return FALSE;
            }
        }
    }
    ...
    PSMWP psmwp)
{
    ...
    for (pcvr = psmwp->acvr, ccvr = psmwp->ccvr; --ccvr >= 0; pcvr++) {
        ...
        UserAssert(!(TestWF(pwnd, WFTOGGLETOPMOST) &&
                (pcvr->pos.flags & SWP_NOZORDER)));
        ...
        if (!(pcvr->pos.flags & SWP_NOZORDER)) {
            if (ValidateWindowPos(&pcvr->pos)) {
                ...
                LinkWindow(pwnd, PWInsertAfter(pcvr->pos.hwndInsertAfter),
        ...
        if (pcvr->pos.flags & SWP_SHOWWINDOW) {
             ...
             * seconds.
             */
            if (GETPTI(pwnd)->ppi->flags & PIF_APPSTARTING)
                CalcStartCursorHide(GETPTI(pwnd)->ppi, 5000);
            ...
            if (!(pcvr->pos.flags & SWP_NOREDRAW)) {
                ...
                if (TestCF(pwnd, CFSAVEBITS)) {
                     ...
                     */
                    PWND pwndT = pwnd->spwndParent->spwndChild;
                    for ( ; pwndT != NULL; pwndT = pwndT->spwndNext) {
                        if (pwndT == pwnd) {
                            CreateSpb(pwnd, FALSE, ghdcScreen);
                            break;
                        }
         ...
         */
        if (!TestWF(pwnd, WFWIN31COMPAT) && TestWF(pwnd, WFVISIBLE) &&
                (pcvr->pos.flags & SWP_NOREDRAW)) {
...
CleanupAndExit:
    ...
    if (PW(ppos->hwnd)->pwindow != NULL) {
         ...
         */
        if (pwndParent != NULL) {
            InvalidateDCCache(pwndParent, TestWF(pwndParent, WFCLIPCHILDREN)
...
ValidEmpty:
        ...
        if (!(fsRgnEmpty & RE_VALID) &&
                ((pcvr->dxBlt | pcvr->dyBlt) != 0)) {
            if (hdcScreen == NULL) {
                hdcScreen = ghdcScreen;
            }

            GreSelectVisRgn(hdcScreen, hrgnValid, NULL, SVR_DELETEOLD | SVR_COPYNEW);
         ...
         */
        if (TestWF(pwnd, WFHASSPB) && !(fsRgnEmpty & RE_VISOLD) &&
                CombineOldNewVis(hrgnInvalid, pcvr->hrgnVisOld,
                hrgnVisNew, RGN_DIFF, fsRgnEmpty)) {
            UINT retRSPB;
            ...
            if (pcvr->hrgnVisOld == NULL) {
                ...
                GreSetRectRgn(hrgnInvalid,
                        pwndParent->rcWindow.left, pwndParent->rcWindow.top,
                        pwndParent->rcWindow.right, pwndParent->rcWindow.bottom);
            } else {
                if (!CombineOldNewVis(hrgnInvalid, pcvr->hrgnVisOld, hrgnVisNew, RGN_OR, fsRgnEmpty))
                    goto InvalidEmpty;
            }
        }
...
InvalidEmpty:
        ...
        if (!(fsRgnEmpty & RE_INVALID)) {
            ...
            if ((fsRgnEmpty & RE_VALID) && !(fsRgnEmpty & RE_VISNEW)) {
                 ...
                 */
                xxxInternalInvalidate(pwnd, (HRGN)1, RDW_INVALIDATE | RDW_FRAME |
...
UnlockAndExit:
    ...
    if (hdcScreen != NULL) {
         ...
         */
        GreSelectVisRgn(hdcScreen, NULL, NULL, SVR_DELETEOLD | SVR_COPYNEW);
    ...
    HRGN hrgn)
{
     ...
     */
    CalcVisRgn(&hrgn, pwnd, pwnd, (TestWF(pwnd, WFCLIPSIBLINGS)
    ...
    UINT fsRgnEmpty)
{
    ...
    case RE_VISOLD:
        ...
        /*
         * If we're calculating old - new and old is empty, then result is empty.
         * Otherwise, result is new.
         */
        if (crgn == RGN_DIFF)
            return FALSE;
...
BOOL ValidateZorder(
    PCVR pcvr)
{
    PWND pwnd, pwndPrev, pwndInsertAfter;
    BYTE bTopmost;
    ...
    PWINDOWPOS ppos)
{
    ...
    if (!(ppos->flags & SWP_NOCLIENTMOVE)) {
        ...
        if (pwndParent != NULL) {
            xxxSendMessage(pwnd, WM_MOVE, FALSE,
    ...
    PWND pwnd)
{
    ...
    if (!TestwndChild(pwnd)) {
        if ((GETPTI(pwnd)->flags & TIF_ALLOWFOREGROUNDACTIVATE) ||
                (GETPTI(pwnd)->ppi->flags & PIF_ALLOWFOREGROUNDACTIVATE)) {
            return GetLastTopMostWindow();
        }
    }
    ...
    PWINDOWPOS ppos)
{
    ...
    if (ppos->hwndInsertAfter == HWND_BOTTOM) {
        ...
        if (TestWF(pwnd, WEFTOPMOST)) {
            ppos->hwndInsertAfter = HW(GetLastTopMostWindow());
            if (ppos->hwndInsertAfter == ppos->hwnd)
                ppos->hwndInsertAfter = HW(_GetWindow(pwnd, GW_HWNDPREV));
        } else {
            ppos->hwndInsertAfter = HW(_GetWindow(pwnd, GW_HWNDPREV));
        }
        return CTM_NOTOPMOST;
     ...
     */
    if (ppos->hwndInsertAfter == HW(GetLastTopMostWindow()))
        return CTM_NOCHANGE;
   ...
   HWND hwnd)
{
    ...
    default:
        ...
        if (pwnd != NULL) {
            ...
            UserAssert(pwnd->spwndParent != NULL &&
                    IsDescendant(pwnd->spwndParent, pwnd));
            return pwnd;
        }
        return NULL;
    }
    ...
    int iTop)
{
    ...
    while ((pwndT = NextOwnedWindow(pwndT, pwnd, pwnd->spwndParent)) != NULL) {
        ...
        if (pwndChgOwnee == NULL) {
            ...
            if (!fChgOwneeInserted && IsBehind(pwndT, pwndInsertAfter)) {
                psmwp = AddSelfAndOwnees(psmwp, pwndChgOwnee, pwndChange,
                        pwndInsertAfter, iTop);
                if (psmwp == NULL)
                    return NULL;
    ...
    if (pwndChgOwnee != NULL && !fChgOwneeInserted) {
        psmwp = AddSelfAndOwnees(psmwp, pwndChgOwnee,
                pwndChange, pwndInsertAfter, iTop);
        if (psmwp == NULL)
            return NULL;
    }
    ...
    int iTop)
{
    ...
    if (!(ppos->flags & SWP_NOZORDER)) {
        ...
        psmwp = AddSelfAndOwnees(psmwp,
                pwndOwnerRoot, pwnd, pwndInsertAfter, iTop);
    ...
    PSMWP psmwp)
{
    ...
    for (i = psmwp->ccvr; i-- != 0; ) {
        ...
        psmwp = ZOrderByOwner2(psmwp, iTop);
        if (psmwp == NULL)
            break;
        ...
        for (iScan = iTop; iScan != psmwp->ccvr; iScan++) {
             ...
             */
            if (GETPTI(pwnd)->flags & TIF_16BIT) {
                if (gptiForeground == NULL)
                    continue;

                if (GETPTI(pwnd)->idProcess == gptiForeground->idProcess)
                    continue;
            } else {
...
BOOL xxxEndDeferWindowPosEx(
    ...
    BOOL fAsync)
{
    PWND pwndNewActive, pwndParent, pwndActive, pwndActivePrev;
    HWND hwndNewActive;
    PWINDOWPOS pwp;
    BOOL fClearBits, fSyncPaint;
    ...
    if ((psmwp->ccvr != 0) && ValidateSmwp(psmwp, &fSyncPaint)) {
        ...
        if (xxxCalcValidRects(psmwp, &hwndNewActive)) {
             ...
             */
            if (fForegroundPrev && cVisWindowsPrev != 0 &&
                    pti->cVisWindows == 0) {
                pti->flags |= TIF_ALLOWFOREGROUNDACTIVATE;
...
BOOL xxxEndDeferWindowPos(
    PSMWP psmwp)
{
    return xxxEndDeferWindowPosEx(psmwp, FALSE);
}
    ...
    PWND pwnd)
{
    ...
    if ((int)GETPTI(pwnd)->cVisWindows < 0) {
        SRIP0(RIP_ERROR, "pti->cVisWindows underflow!");
    }
#endif
}
...
*
* History:
* 07-22-91 darrinm      Ported from Win 3.1 sources.
\***************************************************************************/
