...
VOID NEAR PASCAL HandleStartupGroup()
{
...
#ifdef JAPAN
//We start both of Kana Start and Start whichever not registerd in registry
//v-hitot
                                if( StartupCmp(szStartupKana,PTR(lpgd,lpgd->pName)) || StartupCmp(szStartup,PTR(lpgd,lpgd->pName)) )
                                        StartupGroup( hwndT );
                                else if( pGroup->fCommon ){
#else
                if (pGroup->fCommon) {
#endif
...
{
    if (wcsnicmp(p, TEXT("GROUP"), CCHGROUP) != 0) {
        return FALSE;
    }
...
VOID PASCAL LoadAllGroups()
{
    ...
    RegSetValueEx(hkeyPMSettings, szSystemDisplay, 0, REG_SZ, (LPBYTE)szPath, sizeof(TCHAR)*lstrlen(szPath));
...
//  MarkProcess(MP_SYSTEMAPP);
  ...
  hhkMsgFilter = SetWindowsHook(WH_MSGFILTER, MakeProcInstance((HOOKPROC)MessageFilter, hAppInstance));
  if (hhkMsgFilter == 0) {
      GetLastError();
  }
  /*
