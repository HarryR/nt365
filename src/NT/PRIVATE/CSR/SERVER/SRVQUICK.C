...
PCSR_THREAD CsrConnectToUser( VOID )
{
    ...
    if (ClientThreadConnectRoutine == NULL) {
        ...
        // MicroNT: in a headless-Win32 config (no winsrv/USER), user32.dll
        // isn't present and ClientThreadConnectRoutine remains NULL. The
        // original code called it unconditionally below and EIP-0 crashed.
        // Bail out now and let the caller handle a NULL return as "no
        // USER connection available".
        if (ClientThreadConnectRoutine == NULL) {
            DbgPrint("CSRSS: CsrConnectToUser — user32 not loaded, no USER subsystem\n");
            return NULL;
        }
    }
