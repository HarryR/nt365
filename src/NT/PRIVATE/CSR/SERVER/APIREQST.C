    ...
    )
{
    ...
    Teb = NtCurrentTeb();
    DbgPrint("CSRSRV: CsrApiRequestThread entered tid=%p StackBase=%p StackLimit=%p reserve=%u commit=%u\n",
             Teb->ClientId.UniqueThread,
             Teb->NtTib.StackBase,
             Teb->NtTib.StackLimit,
             (ULONG)((ULONG)Teb->NtTib.StackBase - (ULONG)Teb->DeallocationStack),
             (ULONG)((ULONG)Teb->NtTib.StackBase - (ULONG)Teb->NtTib.StackLimit));
    ReplyMsg = NULL;
...
// (Also see SRVQUICK.C.)
    ...
    //
    // Try to connect to USER. In headless-Win32 (no winsrv) CsrConnectToUser
    // returns NULL immediately and we skip the QlpcTeb setup — the request
    // thread still handles basesrv LPC traffic, it just can't relay to USER.
    //

    {
        /* Try once. If user32 isn't present (headless Win32 config),
         * CsrConnectToUser returns NULL immediately — don't sleep +
         * retry the original 30-sec loop; just run the worker in
         * basesrv-only mode. USER-bound messages can't arrive anyway
         * because we don't load winsrv. */
        PCSR_THREAD connected = CsrConnectToUser();
        if (connected) {
            QlpcTeb = (PCSR_QLPC_TEB)NtCurrentTeb()->CsrQlpcTeb;
            MyThread = QlpcTeb->ClientThread;
        } else {
            DbgPrint("CSRSRV: no USER connection — basesrv-only mode\n");
            QlpcTeb = NULL;
            MyThread = NULL;
        }
    }
