...
GetNextAddrObj(AOSearchContext *SearchContext)
{
    ...
    if (FoundAO != NULL) {
        ...
        // Don't bother to update port, it doesn't change.
    }
    return FoundAO;
}
...
InvalidateAddrs(IPAddr Addr)
{
        ...
        while (AO != NULL) {
			...
			if (IP_ADDR_EQUAL(AO->ao_addr, Addr) && AO_VALID(AO)) {
				...
				while (!EMPTYQ(&AO->ao_sendq)) {
        			DEQUEUE(&AO->ao_sendq, SendReq, UDPSendReq, usr_q);
        			CTEStructAssert(SendReq, usr);
					ENQUEUE(&SendQ, &SendReq->usr_q);
				}
    ...
    while (!EMPTYQ(&RcvQ)) {
        ...
        DEQUEUE(&RcvQ, RcvReq, UDPRcvReq, urr_q);
        CTEStructAssert(RcvReq, urr);

        (*RcvReq->urr_rtn)(RcvReq->urr_context, (uint) TDI_ADDR_INVALID, 0);
    ...
    while (!EMPTYQ(&SendQ)) {
        ...
        DEQUEUE(&SendQ, SendReq, UDPSendReq, usr_q);
        CTEStructAssert(SendReq, usr);

        (*SendReq->usr_rtn)(SendReq->usr_context, (uint) TDI_ADDR_INVALID, 0);

        CTEGetLock(&UDPSendReqLock, &TableHandle);
        if (SendReq->usr_header != NULL)
            FreeUDPHeader(SendReq->usr_header);
        FreeUDPSendReq(SendReq);
        CTEFreeLock(&UDPSendReqLock, TableHandle);
			...
			uint Protocol, void *Ptr)
{
    ...
    // The specified IP address is a valid local address. Now, if no port is
    // specified we have to assign one. If there is a port specified, we need
    // to make sure that the IPAddress/Port combo isn't already open (unless
    // Reuse is specified). If the input address is a wildcard, we need to
    // make sure the address isn't open on any local ip address.
    ...
    // When we get here, we know we're creating a brand new address object. Port
    // contains the port in question, and NewAO points to the newly created AO
    // (hopefully).
    if (NewAO != NULL) {
#ifdef  VXD
        uint    NewAOIndex;
...
DeleteAO(AddrObj *DeletedAO)
{
...
#ifndef UDP_ONLY
    CTEFreeLock(&UDPSendReqLock, AOHandle);
    ...
    while (!EMPTYQ(&DeletedAO->ao_rcvq)) {
        ...
        DEQUEUE(&DeletedAO->ao_rcvq, Rcv, UDPRcvReq, urr_q);
        CTEStructAssert(Rcv, urr);

        CTEFreeLock(&DeletedAO->ao_lock, TableHandle);
        (*Rcv->urr_rtn)(Rcv->urr_context, (uint) TDI_ADDR_DELETED, 0);
    ...
    while (!EMPTYQ(&DeletedAO->ao_sendq)) {
        ...
        DEQUEUE(&DeletedAO->ao_sendq, Send, UDPSendReq, usr_q);
        CTEStructAssert(Send, usr);

        CTEFreeLock(&DeletedAO->ao_lock, TableHandle);
        (*Send->usr_rtn)(Send->usr_context, (uint) TDI_ADDR_DELETED, 0);

        CTEGetLock(&UDPSendReqLock, &HeaderHandle);
        if (Send->usr_header != NULL)
            FreeUDPHeader(Send->usr_header);
        FreeUDPSendReq(Send);
...
SetAOOptions(AddrObj *OptionAO, uint ID, uint Length, uchar *Options)
{
    ...
    // First, see if there are UDP options.
    ...
    while (AO_PENDING(RequestAO))  {
        ...
        if (AO_REQUEST(RequestAO, AO_SEND)) {
            ...
            if (!EMPTYQ(&RequestAO->ao_sendq)) {
                DEQUEUE(&RequestAO->ao_sendq, SendReq, UDPSendReq, usr_q);
