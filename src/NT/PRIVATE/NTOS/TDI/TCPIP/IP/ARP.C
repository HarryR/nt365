...
FreeARPBuffer(ARPInterface *Interface, PNDIS_BUFFER Buffer)
{
	...
	if (Size <= Interface->ai_sbsize) {
    	...
    	ExInterlockedPushEntryList(
        	STRUCT_OF(SINGLE_LIST_ENTRY, &Interface->ai_sblist, Next),
	...
	uchar *SrcAddr)
{
    ...
    do {
		if (!IP_ADDR_EQUAL(Addr->aia_addr, NULL_IP_ADDR)) {
			// This is a valid address. See if he's on our subnet.
			if (IP_ADDR_EQUAL(Addr->aia_addr & Addr->aia_mask,
    ...
    RC UNALIGNED *SourceRoute, uint SourceRouteSize, uint UseSNAP)
{
    ...
    SendARPPacket(Interface, Packet,(ARPHeader *)(Header + Size), net_short(ARP_RESPONSE),
        HWAddress, NULL, Destination, Src, HWType);
}
    ...
    CTELockHandle lhandle)
{
    ...
    if (Interface->ai_state == INTERFACE_UP) {
        ...
        if (entry->ate_state == ARP_GOOD) {     // Entry is valid
			...
			entry->ate_useticks = ATE_TICK_VALUE;
        ...
        CTELockHandle *Handle)
{
    ...
    NewEntry->ate_state = (Interface->ai_media != NdisMedium802_5 ?
        ARP_RESOLVING_GLOBAL : ARP_RESOLVING_LOCAL);
    NewEntry->ate_rce = NULL;

    NewEntry->ate_valid = CTESystemUpTime();
        NewEntry->ate_useticks = ATE_TICK_VALUE;
    ...
    if (entry != NULL) {
        if (entry->ate_state <= ARP_RESOLVING) {                // Newly created entry.
			...
			if (entry->ate_packet == NULL) {
                entry->ate_packet = Packet;
                CTEFreeLock(&entry->ate_lock, lhandle);
                SendARPRequest(ai, Destination, entry->ate_state, NULL);
...
//
uint
ARPAddAddr(void *Context, uint Type, IPAddr Address, IPMask Mask)
{
    ARPInterface    *Interface = (ARPInterface *)Context;
    CTELockHandle   Handle;
    ...
    if (Type != LLIP_ADDR_LOCAL && Type != LLIP_ADDR_PARP) {
		...
		if (Type != LLIP_ADDR_PARP) {
			...
			if (RetStatus == TRUE && Interface->ai_state == INTERFACE_UP) {
...
{
#ifdef	VXD
	uchar			IPAddrBuffer[(sizeof(IPAddr) * 4) - 1];
	uchar			HWAddrBuffer[(ARP_802_ADDR_LENGTH * 3) - 1];
	uint			i;
    ...
    ARPHeader UNALIGNED *ARPHdr, uint ARPHdrSize, uint ProtOffset)
{
    ...
    if (Interface->ai_media == NdisMedium802_5) // Figure out SR size on TR.
        // Check for source route information. SR is present if the header size
        // is greater than the standard TR header size. If the SR is only an RC
        // field, we ignore it because it came from the same ring which is the
        // same as no SR.

        if ((HeaderSize - sizeof(TRHeader)) > sizeof(RC)) {
            SourceRouteSize = HeaderSize - sizeof(TRHeader);
            SourceRoute = (RC UNALIGNED *)((uchar *)Header + sizeof(TRHeader));
        }
    ...
    if (Entry != (ARPTableEntry *)NULL) {
		...
		switch (Interface->ai_media) {
			...
			case NdisMedium802_5:
				...
				if (Entry->ate_state != ARP_GOOD ||
					(Entry->ate_valid - Now) > ARP_RESOLVE_TIMEOUT) {
    ...
    ENetHeader          *Header;
        ...
        if (ID->toi_class == INFO_CLASS_IMPLEMENTATION) {
                ...
                if (PArpEntry->pae_status == PAE_STATUS_VALID) {
                        ...
                        if (ARPAddAddr(Interface, LLIP_ADDR_PARP, AddAddr, Mask))
                                return TDI_SUCCESS;
                        else
...
// If we're bound to the adapter, close it now.
    ...
    if (Interface->ai_ARPTbl != (ARPTable *)NULL)
        CTEFreeMem(Interface->ai_ARPTbl);
	...
	NDIS_HANDLE UnbindContext)
{
	...
	*RetStatus = Status;
	...
	// Now free the memory for this interface. First we'll free any outstanding
	// ARP table entries.
