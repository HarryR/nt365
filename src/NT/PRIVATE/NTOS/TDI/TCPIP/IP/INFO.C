...
IPSetInfo(TDIObjectID *ID, void *Buffer, uint Size)
{
	...
	if (ID->toi_id == IP_MIB_RTTABLE_ENTRY_ID) {
		...
		if (IRE->ire_type != IRE_TYPE_INVALID) {
			// He's adding a route.
			Status = AddRoute(Dest, IRE->ire_mask, FirstHop, OutNTE->nte_if,
				OutNTE->nte_mss, IRE->ire_metric1, IRE->ire_proto,
				ATYPE_OVERRIDE);

		} else {
			// He's deleting a route.
			Status = DeleteRoute(Dest, IRE->ire_mask, FirstHop, OutNTE->nte_if);
		}
