...
//* Global IP ID.
DEFINE_LOCK_STRUCTURE(IPIDLock)
ulong  IPID;
...
FreeIPHdrBuffer(PNDIS_BUFFER Buffer)
{
    ...
    ExInterlockedPushEntryList(
        STRUCT_OF(SINGLE_LIST_ENTRY, &HdrBufList, Next),
...
GetIPPacket(void)
{
    ...
    Link = ExInterlockedPopEntryList(
                     STRUCT_OF(SINGLE_LIST_ENTRY, &PacketList, Next),
                     &HeaderLock
                     );
    if (Link != NULL) {
	...
	IPHeader *IPH)
{
	...
	for (NTE = NetTableList; NTE != NULL; NTE = NTE->nte_next) {
		if (!(NTE->nte_flags & NTE_VALID)) {
			// Found an invalid NTE. Send on that interface.
			return SendIPPacket(NTE->nte_if, Dest, Packet, 	Buffer, IPH,
				NULL, 0);
		}
	}
...
//	decision on where to send datagrams into one spot. If SendOnSource if FALSE
    ...
    IPAddr Dest, IPAddr Source, IPOptInfo *OptInfo, RouteCacheEntry *RCE)
{
    ...
    if ((Packet = GetIPPacket()) != (PNDIS_PACKET)NULL) {     // Got a packet.
                ...
                if (OptInfo->ioi_options == (uchar *)NULL) {
                    if (RCE->rce_dtype != DEST_BCAST)  {
                        if (DataSize <= MTU) {
					        ...
					        IPH->iph_protocol = PInfo->pi_protocol;
					        IPH->iph_xsum = 0;
					        IPH->iph_dest = Dest;
					        IPH->iph_src = Source;
					        ...
					        IPH->iph_offset =
					        	net_short(((OptInfo->ioi_flags & IP_FLAG_DF)
					        		<< 13));
					        IPH->iph_id =
					        	(ushort)CTEInterlockedAddUlong(&IPID, 1,
					        	&IPIDLock);
        ...
        IPH->iph_id = (ushort)CTEInterlockedAddUlong(&IPID, 1, &IPIDLock);
        pc = (PacketContext *)Packet->ProtocolReserved;
        pc->pc_common.pc_flags |= PACKET_FLAG_IPHDR;
        ...
        if (IP_ADDR_EQUAL(FirstHop, NULL_IP_ADDR)) {
			...
			DestIF = LookupNextHop(Dest, Source, &FirstHop, &MTU);
            if (DestIF == NULL) {
        ...
        if (DType == DEST_BCAST) {
			if (IP_ADDR_EQUAL(Source, NULL_IP_ADDR))
				return SendDHCPPacket(Dest, Packet, Buffer, IPH);
			else
            	return SendIPBCast(NULL, Dest, Packet, IPH, Buffer, DataSize,	
        ...
        return SendIPPacket(DestIF, FirstHop, Packet, Buffer, IPH, Options,
            OptionSize);
    }
