...

{
    ...
    DbgPrint("SERIAL: DriverEntry reached\n");
    ...
    DbgPrint("SERIAL: DriverEntry DeviceObject=%p\n", DriverObject->DeviceObject);
...

{
    ...
    DbgPrint("SERIAL: SerialInitializeController entry port=0x%x irq=%u\n",
             ConfigData->Controller.LowPart, ConfigData->OriginalVector);
    ...
    DbgPrint("SERIAL: KdComPortInUse deref=%p vs port=0x%x\n",
             *KdComPortInUse, ConfigData->Controller.LowPart);
    if ((*KdComPortInUse) ==

        ((PUCHAR)(ConfigData->Controller.LowPart))) {
        DbgPrint("SERIAL: reject - KD using port\n");
        SerialDump(
            SERERRORS,
    ...
    DbgPrint("SERIAL: calling IoCreateDevice\n");
    ...
    DbgPrint("SERIAL: IoCreateDevice status=0x%lx dev=%p\n",
             status, deviceObject);
    ...
    if (!extension->Controller) {
        ...
        DbgPrint("SERIAL: bail - SerialGetMappedAddress failed\n");
        goto ExtensionCleanup;
    ...
    if (conflictDetected) {
        ...
        DbgPrint("SERIAL: bail - resource conflict on SerialReportResourcesDevice\n");
        goto ExtensionCleanup;
    ...
    if (!SerialDoesPortExist(
        ...
        DbgPrint("SERIAL: bail - SerialDoesPortExist failed\n");
        goto ExtensionCleanup;
...

{
    ...
    DbgPrint("SERIAL: DLAB probe value1=0x%04x value2=0x%04x oldLCR=0x%02x oldIER=0x%02x\n",
             value1, value2, oldLCRContents, oldIERContents);
...

{
    ...
    DbgPrint("SERIAL: ConfigCallBack BusType=%u BusNum=%u CtrlNum=%u "
             "CtrlInfo[0].DataLength=%u\n",
             BusType, BusNumber, ControllerNumber,
             ControllerInformation[IoQueryDeviceConfigurationData]
                 ? ControllerInformation[IoQueryDeviceConfigurationData]->DataLength
                 : 0xFFFFFFFF);
    ...
    DbgPrint("SERIAL: ConfigCallBack foundPort=%u foundInterrupt=%u "
             "port=0x%lx irq=%u\n",
             (ULONG)foundPort, (ULONG)foundInterrupt,
             controller->Controller.LowPart,
             controller->OriginalVector);
