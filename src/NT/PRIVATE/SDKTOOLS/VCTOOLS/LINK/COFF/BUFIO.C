...

{
    ...
    if (cbSizeNew > pfi->cbMapView) {
        ...
        if (!ExtendMapView(pfi, cbSizeNew)) {
            Error(pfi->szFileName, CANTSEEKFILE, cbSizeNew);
        }
    } else {
        // Shrink file
...

{
    ...
    if (fNewPfi) {
                              ...
                              ((flags & (_O_RDWR|_O_WRONLY)) ? _SH_DENYRW : _SH_DENYWR), mode)) == -1)
        {
            ...
            Error(NULL, CANTOPENFILE, pfi->szFileName);
        }
...

{
    ...
    if ((i = _close(pfi->fd)) == -1) {
        Error(NULL, CANTCLOSEFILE, pfi->szFileName);
    }
...

{
    ...
    if (pbuf->flags & BUF_Dirty) {
        ...
        if (_lseek(pfi->fd, pbuf->ibStart, SEEK_SET) == -1L) {
            Error(pfi->szFileName, CANTSEEKFILE, pbuf->ibStart);
        }

        // flush buffer
        if (_write(pfi->fd, pbuf->rgbBuf, cb) != cb) {
            Error(pfi->szFileName, CANTWRITEFILE, _tell(pfi->fd));
        }
...

{
    ...
    if ((pfi->pbufCur->ibCur + cb) > (ULONG)pfi->cbMap) {
        Error(pfi->szFileName, CANTREADFILE, pfi->pbufCur->ibCur + cb);
    }
...

{
    ...
    if ((ibSeek = (LONG)_lseek(pfi->fd,
        ...
        Error(pfi->szFileName, CANTSEEKFILE, ibSeek);
    }
    ...
    if ((cbRead = _read(pfi->fd, pfi->pbufCur->rgbBuf,
        ...
        Error(pfi->szFileName, CANTREADFILE, _tell(pfi->fd));
    }
...

{
    ...
    /* MicroNT patch (for wibo): the original asserts this buffer is not
     * BUF_Current. In the shrink-file path (BufferedChSize) a pfi->pbufCur
     * can reach here with BUF_Current still set. Clear it defensively so
     * FlushBuffer + the reset below leave the PFI's pbufCur pointer
     * dangling (expected after deactivation). */
    if (pbuf->flags & BUF_Current) {
        pbuf->flags &= ~BUF_Current;
    }
...

{
    ...
    if (pfi->ibCur > pfi->cbMapView) {
        if (!ExtendMapView(pfi, pfi->ibCur)) {
            if (pfi->MapAddr == 0) {
                Error(pfi->szFileName, CANTSEEKFILE, pfi->ibCur);
            }
    ...
    if ((pfi->ibCur + (LONG) cb) > pfi->cbMapView) {
        if (pfi->flags & FI_Write) {
            if (!ExtendMapView(pfi, pfi->ibCur + (LONG) cb)) {
                Error(pfi->szFileName, CANTSEEKFILE, pfi->ibCur + (LONG) cb);
            }
        } else {
            return(0);
        }
...

{
    ...
    // UNDONE: Does it make sense when RELEASE is set to FlushViewOfFile()
    //          before unmapping?  What's more important at that time is that the
    //          bits are on disk in the right place...  Or does the subsequent
    //          call to CheckSum the image do this for us...
...
PbMappedRegion(IN INT fd, IN ULONG ibStart, IN ULONG cb)
{
    ...
    if (ibStart + cb > (ULONG)pfi->cbMapView) {
        if (!ExtendMapView(pfi, ibStart + cb)) {
            Error(pfi->szFileName, CANTSEEKFILE, ibStart + cb);
        }
    }
