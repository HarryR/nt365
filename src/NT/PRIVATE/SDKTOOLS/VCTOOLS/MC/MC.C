...
{
    /* Original stock MC converts every argv[] from ANSI to OEM via
     * user32!CharToOem so filenames match DOS codepage expectations.
     * Wibo does not implement user32!CharToOemA; our build only feeds
     * MC ASCII filenames (e.g. BUGCODES.MC, winerror.mc), and for pure
     * ASCII CharToOem is an identity mapping. Drop the call entirely.
     * SetFileApisToOEM is also left out — it only affects how the
     * kernel32 Ansi<->Oem conversion runs on subsequent file APIs, and
     * with ASCII input the distinction doesn't matter. */
    (void)argc;
    (void)argv;
}
