...
#include "loader.h"
#include "data/disk.h"   /* BOOT_DISK_SIGNATURE / BOOT_DISK_CHECKSUM from mkdisk.py */
...
/* Boot disk ARC signature entry. Values come from data/disk.h which is
 * generated by tools/mkdisk.py, so the checksum matches the image on
 * disk exactly. IopCreateArcNames uses this to create:
...
static char BootDiskArcName[]  = "multi(0)disk(0)rdisk(0)";
static ARC_DISK_SIGNATURE BootDiskSignature;
...
static void build_loader_block(multiboot_info_t *mbi) {
    ...
    /* ARC disk info — one entry for our mkdisk-built boot disk. */
    InitializeListHead(&ArcDiskInfo.DiskSignatures);
    {
        BootDiskSignature.Signature           = BOOT_DISK_SIGNATURE;
        BootDiskSignature.ArcName             = BootDiskArcName;
        BootDiskSignature.CheckSum            = BOOT_DISK_CHECKSUM;
        BootDiskSignature.ValidPartitionTable = 1;   /* TRUE */
        /* Tail-insert into DiskSignatures. Pointers KSEG0-fixed up later. */
        LIST_ENTRY *e = &BootDiskSignature.ListEntry;
        LIST_ENTRY *h = &ArcDiskInfo.DiskSignatures;
        e->Flink = h;
        e->Blink = h->Blink;
        h->Blink->Flink = e;
        h->Blink = e;
...
ULONG loader_main(multiboot_info_t *mbi) {
     ...
     */
    {
        ...
        /* ArcDiskInfo.DiskSignatures list — its entries have our BootDiskSignature
         * linked in and its ArcName string points at BootDiskArcName[]. Walk
         * the list and KSEG0-ify Flink/Blink and ArcName.
         * FIXLIST already handles Flink/Blink; we fix ArcName manually. */
        FIXLIST(&ArcDiskInfo.DiskSignatures);
        BootDiskSignature.ArcName = TOKSEG(BootDiskSignature.ArcName);
